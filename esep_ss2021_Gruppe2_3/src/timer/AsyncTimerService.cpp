...
void AsyncTimerService::handle(dispatcher::Event& event) {
    if(event.type == dispatcher::EventType::EVNT_TIM_REQ){
        ...
        //uint16_t coid = _dispatcher_connection->get_id();
        //SIGEV_PULSE_INIT(&return_event, coid, SIGEV_PULSE_PRIO_INHERIT, static_cast<int>(dispatcher::EventType::EVNT_TIM_ALRT), alrt_val);
