...
class SimGPIO {
private:
	ISimulationImageAccess *simulation;
	unsigned int bank0_OE = 0xFFFFFFFF;   // all pin are input, spruhl73l, p4893
	unsigned int bank1_OE = 0xFFF00FFF; // pin are allready set to output by system.
...
public:
	...
	bool checkRasingIRQADC();
	void updateDataADC(unsigned int rawvalue);

	static SimGPIO* getGPIO() {
		static SimGPIO instance;
		return &instance;
	}
};
