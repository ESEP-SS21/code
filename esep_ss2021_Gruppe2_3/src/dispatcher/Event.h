...
enum class EventType { //make sure to add a string representation for each value
    ...
    EVNT_ACT_CTRL_Q1_LED_ON,
    EVNT_ACT_CTRL_Q1_LED_OFF,
    EVNT_ACT_CTRL_Q2_LED_ON,
    EVNT_ACT_CTRL_Q2_LED_OFF,
    EVNT_ACT_BELT_BWD,
    EVNT_ACT_BELT_FWD,
    EVNT_ACT_BELT_STP,
    EVNT_ACT_SORT_DSC,
...
inline std::ostream& operator<<(std::ostream& out, const EventType& e) {
    static const std::string strs[static_cast<int>(EventType::LENGTH)] = {
            "EVNT_ERR",
            "EVNT_ERR_GONE",
            "EVNT_WRN",
            "EVNT_WRN_GONE",
